(function (app) {
  ...
  let metadata = {
    urls: {
      github:
        "https://github.com/bradyhouse/house/tree/master/fiddles/three/fiddle-0005-EarthMoon",
      earth: {
        normalMaterial: "textures/earthSurfaceNormal.jpg",
        surfaceMaterial: "textures/EarthMapAtmos_2500x1250.jpg",
        specularMaterial: "textures/earthSurfaceSpecular.jpg",
        cloudsMaterial: "textures/Transparent_Clouds_Map_Without_Asia.png",
      },
  ...
  app.toolkit.three.R = class extends app.toolkit.three.Publisher {
    ...
    constructor(config) {
      super(config);
      this._objects =
        config && config.hasOwnProperty("objects")
          ? config.objects
          : this.config().objects;
      this._handleKeyUp =
        config && config.hasOwnProperty("keyUp")
          ? config.keyUp
          : this.config().keyUp;
      this._handleKeyDown =
        config && config.hasOwnProperty("keyDown")
          ? config.keyDown
          : this.config().keyDown;
      this._handleKeyPress =
        config && config.hasOwnProperty("keyPress")
          ? config.keyPress
          : this.config().keyPress;
    ...
    objectFromMouse(pagex, pagey) {
      ...
      if (intersects.length > 0) {
        ...
        let mat = new THREE.Matrix4().getInverse(
  ...
  app.view.milkyway.Stars = class extends app.toolkit.three.Object {
    ...
    constructor(config) {
      super();
      this._autoInit =
        config && config.hasOwnProperty("autoInit")
          ? config.autoInit
          : this.config().autoInit;
      this._verticeCount =
        config && config.hasOwnProperty("verticeCount")
          ? config.verticeCount
          : this.config().verticeCount;
      this._materialCount =
        config && config.hasOwnProperty("materialCount")
          ? config.materialCount
          : this.config().materialCount;
      this._systemCount =
        config && config.hasOwnProperty("systemCount")
          ? config.systemCount
          : this.config().systemCount;
      this._distance =
        config && config.hasOwnProperty("distance")
          ? config.distance
    ...
    init() {
      ...
      for (let i = 0; i < this.systemCount; i++) {
        let stars = new THREE.ParticleSystem(
          starsGeometry,
          starsMaterials[i % this.materialCount]
        );
    ...
    constructor(config) {
      ...
      this._autoInit =
        config && config.hasOwnProperty("autoInit")
          ...
          : this.config().autoInit;
      this._size =
        config && config.hasOwnProperty("size")
          ? config.size
          : this.config().size;
      this._distance =
        config && config.hasOwnProperty("distance")
          ...
          : this.config().distance;
      this._period =
        config && config.hasOwnProperty("period")
          ? config.period
          : this.config().period;
      this._map =
        config && config.hasOwnProperty("map") ? config.map : this.config().map;
      this._revolutionSpeed =
        config && config.hasOwnProperty("revolutionSpeed")
          ? config.revolutionSpeed
          : this.config().revolutionSpeed;
      this._animateOrbit =
        config && config.hasOwnProperty("animateOrbit")
          ? config.animateOrbit
          : this.config().animateOrbit;
      this._animateRotation =
        config && config.hasOwnProperty("animateRotation")
          ? config.animateRotation
  ...
  app.view.milkyway.earth.Earth = class extends app.view.milkyway.Planet {
    ...
    update() {
      ...
      if (this.cloudsMesh) {
        ...
        this.cloudsMesh.rotation.x += 0.001;
      }
      this.updateChildren();
    }
    init() {
    ...
    initSurface() {
      let surfaceMap = THREE.ImageUtils.loadTexture(
          metadata.urls.earth.surfaceMaterial
        ),
        normalMap = THREE.ImageUtils.loadTexture(
          metadata.urls.earth.normalMaterial
        ),
        specularMap = THREE.ImageUtils.loadTexture(
          metadata.urls.earth.specularMaterial
        ),
      ...
      let shaderMaterial = new THREE.ShaderMaterial({
        ...
        globeGeometry = new THREE.SphereGeometry(2, 64, 32);
    ...
    initAtmosphere() {
      let cloudsMap = THREE.ImageUtils.loadTexture(
          metadata.urls.earth.cloudsMaterial
        ),
        ...
        cloudsGeometry = new THREE.SphereGeometry(2, 64, 32),
  ...
  app.view.milkyway.earth.Moon = class extends app.toolkit.three.Object {
    ...
    constructor(config) {
      super();
      this._earth =
        config && config.hasOwnProperty("earth")
          ? config.earth
          : this.config().earth;
    }
    get earth() {
  ...
  app.controller = app.controller || {
    onDOMContentLoaded: function () {
      let hook = document.getElementsByTagName("earth")[0],
  ...
  app.test = window.location.pathname.match("test")
    ? app.test || {
        publisher: function () {
